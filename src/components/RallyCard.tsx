...
export default function RallyCard({ rally, onDeleted }: RallyCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Comments panel */}
      {!showComments && rally.commentsCount > 0 && comments && comments.length > 0 && (
        <div className="mt-4 text-sm text-zinc-600 bg-zinc-50/50 p-3 rounded-xl border border-zinc-100/80">
          <div className="flex items-center justify-between mb-2">
            <span className="font-bold text-zinc-800 text-xs tracking-wide uppercase">
              Comments ({rally.commentsCount})
            </span>
          ...
          >
            View all {rally.commentsCount} comments
          </button>
        </div>
      )}
